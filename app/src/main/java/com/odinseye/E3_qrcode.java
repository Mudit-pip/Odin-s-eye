...
public class E3_qrcode extends AppCompatActivity {
                                ...
                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                    if (snapshot.exists()) {
                                        ...
                                        if (um.getScans() > 0) {
                                            ...
                                            add_data_to_firbase(cal.getTime().toString(), um.getNumber(), um.getQuestion(), um.getValue(), um.getScans(), um.getPoints());

                                            ///////////////update scans//////////////////////////////////////
                                            decrease_scans(um.getAnswer(), um.getNumber(), um.getPoints(), um.getQuestion(), um.getScans(), um.getValue());
                                            ////////////////////////////////////////////////////

                                            startActivity(new Intent(E3_qrcode.this, E1_afterlogin.class));
                                            finish();
                                        } else {
                                            startActivity(new Intent(E3_qrcode.this, E4_scansare_over.class));
                                            finish();
                                        }
                                    }
                                }
    ...
    public void decrease_scans(String answer, String number, int points, String question, int scans, String value) {
        db_fire = FirebaseDatabase.getInstance();
        auth = FirebaseAuth.getInstance();
        FirebaseUser user = auth.getCurrentUser();
        ...
        qrdata data = new qrdata(number, question, value, scans, answer, null, points);
        if (user != null) {
            try {
    ...
    public void add_data_to_firbase(String time, String number, String question, String value, int scans, int points) {
        db_fire = FirebaseDatabase.getInstance();
        auth = FirebaseAuth.getInstance();
        FirebaseUser user = auth.getCurrentUser();


        qrdata data = new qrdata(number, question, value, scans, null, time, points);
        if (user != null) {
            try {
